...
impl Wallet {
    ...
    pub fn copy(&self) -> Self {
        Self {
            chains: self.chains.iter().map(|(k, v)| (k.clone(), v.copy())).collect(),
            unassigned_key_pairs: self.unassigned_key_pairs.iter().map(|(k, v)| (k.clone(), v.copy())).collect(),
            default: self.default.clone(),
            genesis_config: self.genesis_config.clone(),
            testing_prng_seed: self.testing_prng_seed.clone(),
        }
    }
...
impl UserChain {
    ...
    pub fn copy(&self) -> Self {
        Self {
            ...
            key_pair: self.key_pair.as_ref().map(KeyPair::copy),
